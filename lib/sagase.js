  ...
  return new Promise(function (resolve, reject) {
    ...
    fs.readdir(opts.folder, function (err, files) {
      ...
      async.each(
        files,
        function (file, cb) {
          var fullPath = path.resolve(opts.folder + '/' + file).replace(process.cwd(), '.')
...
function formatOptions (opts) {
  ...
  var folder = (_.isString(opts.folder) ? opts.folder.replace(/\/$/, '') : '').trim(),
      ...
      nameOnly = _.isBoolean(opts['name-only']) ? opts['name-only'] : true,
      excludeNameOnly = _.isBoolean(opts['exclude-name-only']) ? opts['exclude-name-only'] : true,
